...
from app.services.code_engine.local_slm import local_slm
...
def process_business_query(
    ...
    # -----------------------------------------------------------------
    # STEP 3.5: Check Local Offline SLM (Fine-Tuned Qwen2.5-Coder)
    # -----------------------------------------------------------------
    slm_match = local_slm.generate_solution(raw_query, df, detected_language=lang)
    if slm_match and slm_match.get("python_code"):
        sandbox_res = CodeSandbox.execute(slm_match["python_code"], df)
        if sandbox_res["success"]:
            summary_val = ""
            if isinstance(sandbox_res["result"], dict):
                summary_val = str(sandbox_res["result"].get("value") or sandbox_res["result"].get("formatted") or "")
            ...
            DistillationManager.record_solution(
                ...
                dataset_id=dataset_id,
                query=raw_query,
                intent_label=slm_match["intent_label"],
                python_code=slm_match["python_code"],
                explanation=slm_match["human_explanation"],
                language=slm_match["language"],
                execution_success=True,
                source="local_slm",
                columns=columns_list,
                result_summary=summary_val,
            ...
            query_record = AnalystQuery(
                dataset_id=dataset_id,
                query_text=raw_query,
                structure={"intent": slm_match["intent_label"], "source": "local_slm"},
                evidence_package={
                    "result": sandbox_res["result"],
                    "code": slm_match["python_code"],
                    "observations": [],
                },
            )
            db.add(query_record)
            ...
            return QueryResponse(
                ...
                routing_info=RoutingInfo(
                    route="AUTOMATED_EXECUTION",
                    confidence=0.95,
                    resolved_by="local_slm",
                    escalated=False,
                ),
                narrative=NarrativeResponse(
                    headline=slm_match["human_explanation"],
                    narrative_text=slm_match["human_explanation"],
                    key_takeaways=[
                        f"Task: {slm_match['intent_label']}",
                        "Generated by Local Offline SLM (0 API calls)",
                    ],
                    language_detected=slm_match["language"],
                ),
                generated_code=slm_match["python_code"],
                code_source="local_slm",
                code_result=sandbox_res["result"],
                human_explanation=slm_match["human_explanation"],
                execution_time_ms=sandbox_res["execution_time_ms"],
            )
...
def get_slm_status():
    """Returns whether the local offline SLM model is detected and ready."""
    return local_slm.get_status()
